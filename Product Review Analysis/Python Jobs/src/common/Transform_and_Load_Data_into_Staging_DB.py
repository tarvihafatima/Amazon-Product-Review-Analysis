from pyspark.sql.functions import col, lower, trim, expr, initcap,regexp_replace, \
                                  split,to_date, length, when, from_json, posexplode, current_timestamp
...
def get_postgres_url_properties(database, server, port):
    # Set Postgres credentials and connection properties
    postgres_url = "jdbc:postgresql://" + server + ":" + str(port) + "/" + database
    return postgres_url

def extract_data(spark, postgres_url, schema, table_name, username, password):
    # Extract data from Postgres table
    print(spark, postgres_url, schema, table_name, username, password)
    df = spark.read \
        .format("jdbc") \
        .option("url", postgres_url + "?currentSchema=" + schema+ "&user=" + username + "&password=" + password) \
...
# Function to store the processed data back into PostgreSQL
